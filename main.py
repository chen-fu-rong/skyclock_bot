...
# === ✅ DATABASE SETUP (Render-optimized) ===
...
async def webhook(request: Request):
    ...
    asyncio.create_task(process_update(data))  # Offload processing
    return {"ok": True}
...
# === ✅ EVENT TIME CALCULATIONS (Improved) ===
    ...
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            ...
            for user_id, event in notifications:
                ...
                # Implementation would calculate exact notification time
...
if not IS_RENDER:
    ...
    if job_queue:
        job_queue.run_repeating(check_scheduled_events, interval=60, first=10)

...
def wax_event_keyboard(event_type: str):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔔 Notify Me", callback_data=f"notify_{event_type}")],
        [InlineKeyboardButton("🔕 Turn Off Notification", callback_data=f"cancel_notify_{event_type}")],
...
async def wax_event_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    times = EVENT_TIMES.get(event, [])
    formatted = "\n".join(f"• {t}" for t in times)
    await query.edit_message_text(
        f"📅 Next {event.title()} times today:\n{formatted}\n\n🔔 Want a reminder?",
        reply_markup=wax_event_keyboard(event)
    )
...
async def notify_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "\n".join(f"• {t}" for t in times)
    await query.edit_message_text(f"Please reply with the event time you want a reminder for:\n{text}")

async def cancel_notification_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    event_type = query.data.replace("cancel_notify_", "")
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("SELECT job_id FROM reminders WHERE user_id = $1 AND event = $2", user_id, event_type)
        for row in rows:
            scheduler.remove_job(row['job_id'])
        await conn.execute("DELETE FROM reminders WHERE user_id = $1 AND event = $2", user_id, event_type)
    await query.edit_message_text(f"🔕 Notifications for {event_type.title()} turned off.", reply_markup=wax_event_keyboard(event_type))
...
async def load_reminders():
    async with db_pool.acquire() as conn:
        ...
        for row in rows:
            await schedule_reminder(row['user_id'], row['event'], row['event_time'], row['minutes_before'], row['timezone'])
...
async def handle_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = await get_user(user_id)
    if not user or event not in EVENT_TIMES or event_time not in EVENT_TIMES[event]:
        await update.message.reply_text("❌ Invalid time. Try again.")
        return
    await update.message.reply_text("How many minutes before the event would you like to be reminded?")
...
async def list_reminders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("SELECT id, event, event_time, minutes_before FROM reminders WHERE user_id = $1", user_id)
        if not rows:
            await update.message.reply_text("📃 You have no scheduled reminders.")
            return
        for row in rows:
            text = (
                f"🔔 <b>{row['event'].title()}</b>\n"
                f"🕒 Event Time: <b>{row['event_time']}</b>\n"
                f"⏱ Notify: <b>{row['minutes_before']} mins before</b>"
            )
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("❌ Cancel", callback_data=f"cancel_one_{row['id']}")]
            ])
            await update.message.reply_text(text, reply_markup=keyboard, parse_mode=ParseMode.HTML)

async def cancel_one_reminder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    reminder_id = int(query.data.replace("cancel_one_", ""))
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow("SELECT job_id FROM reminders WHERE id = $1", reminder_id)
        if row:
            scheduler.remove_job(row['job_id'])
            await conn.execute("DELETE FROM reminders WHERE id = $1", reminder_id)
            await query.edit_message_text("❌ Reminder canceled.")
        else:
            await query.edit_message_text("⚠️ Reminder not found.")
...
telegram_app.add_handler(CallbackQueryHandler(notify_handler, pattern="^notify_"))
telegram_app.add_handler(CallbackQueryHandler(cancel_notification_handler, pattern="^cancel_notify_"))
telegram_app.add_handler(CallbackQueryHandler(cancel_one_reminder, pattern="^cancel_one_"))
telegram_app.add_handler(MessageHandler(filters.TEXT & filters.TEXT, lambda u, c: handle_minutes_reply(u, c) if c.user_data.get('stage') == 'awaiting_minutes' else handle_reply(u, c)))
...
>>>>>>> 54d0af397d739b058d10caf077f2b8099f9010c8
